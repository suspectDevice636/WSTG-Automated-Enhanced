...
if [ ${SCAN_ENABLED[gobuster]} -eq 1 ] && check_tool gobuster; then
    ...
    run_scan "Gobuster directory scan" "gobuster dir -u $TARGET --wordlist /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -k --status-codes \"204,301,302,307,401,403\" --exclude-length \"\"" "$OUTPUT_DIR/web/04-gobuster-dirs.txt"
fi
