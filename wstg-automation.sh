...
if check_tool curl; then
    run_scan "Directory listing check" "curl -s $TARGET/ > $OUTPUT_DIR/web/03-dir-listing-check.txt 2>&1 && grep -i 'index of|directory listing' $OUTPUT_DIR/web/03-dir-listing-check.txt > /dev/null && echo 'VULNERABLE: Directory listing detected' || echo 'OK: No directory listing'" "$OUTPUT_DIR/web/03-dir-listing-check.txt"
